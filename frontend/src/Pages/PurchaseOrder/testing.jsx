...
// === CHANGE: Added useDebounce hook to debounce search inputs for Vendor and Store to prevent rapid API calls and blinking ===
...
const EditPurchaseOrderDialog = ({
  ...
  // === CHANGE: Added state for Vendor and Store search functionality ===
  ...
  useEffect(() => {
    if (selectedPo) {
      ...
      // === CHANGE: Initialize vendors and stores with selected data to display current vendor/store ===
  ...
  // === CHANGE: Added effect to fetch vendors when debounced search input has 2+ characters ===
  useEffect(() => {
  ...
  // === CHANGE: Added effect to fetch stores when debounced search input has 2+ characters ===
  useEffect(() => {
  ...
  const handleVendorChange = useCallback((event, newValue) => {
    ...
    // === CHANGE: Update vendorSearch to keep input in sync with selection ===
    setVendorSearch(newValue ? newValue.name : "");
  ...
  const handleStoreChange = useCallback((event, newValue) => {
    ...
    // === CHANGE: Update storeSearch to keep input in sync with selection ===
    setStoreSearch(newValue ? newValue.name : "");
          ...
          <>
            ...
            >
              ...
              <TextField
                ...
                sx={{ bgcolor: "#ffffff", borderRadius: "8px" }}
              />
              {/* === CHANGE: Updated Vendor Autocomplete to use search-based vendors and show "No options" === */}
              <Autocomplete
                options={vendors}
              ...
              />
              {/* === CHANGE: Updated Store Autocomplete to use search-based stores and show "No options" === */}
              <Autocomplete
                options={stores}
                getOptionLabel={(option) => option.name || ""}
